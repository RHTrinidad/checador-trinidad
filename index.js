...
const GRUPOS_GERENTES = [GRUPO_COYOACAN_ID, GRUPO_BUCARELI_ID, GRUPO_JUAREZ_ID].filter(Boolean)
const GRUPOS_CHECADORES = [GRUPO_CHECADOR_COYOACAN_ID, GRUPO_CHECADOR_BUCARELI_ID].filter(Boolean)
...
async function getFiltroPorGrupo(jid, sock){
  ...
  try{ const meta = await sock.groupMetadata(jid); const subj = (meta.subject || '').toLowerCase(); let filtro = null; if(subj.includes('coyo')) filtro = 'coyoacan'; else if(subj.includes('bucareli') || subj.includes('juarez')) filtro = 'juarez'; grupoFiltroCache.set(jid, filtro); return filtro }catch{ return null }
}
...
function scoreEmpleado(corto, completo, buscarNorm){ const c = normaliza(corto); const d = normaliza(completo); const cWords = c.split(/\s+/); const dWords = d.split(/\s+/); const allWords = [...cWords,...dWords]; if(c === buscarNorm) return 100; if(c.startsWith(buscarNorm)) return 90; if(cWords.some(w=>w.startsWith(buscarNorm))) return 80; if(dWords.some(w=>w.startsWith(buscarNorm))) return 70; if(d.startsWith(buscarNorm)) return 60; if(allWords.includes(buscarNorm)) return 50; if(buscarNorm.length <= 3) return -1; if(c.includes(buscarNorm) || d.includes(buscarNorm)) return 10; return -1 }
...
async function autocierreAsistencia(){ const sClient = await sheetsClient(); const asisRows = await getRows('Asistencia!A2:M'); const ahoraMX = new Date(new Date().toLocaleString('en-US',{timeZone:'America/Mexico_City'})); for(let i=1; i<asisRows.length; i++){ const r=asisRows[i]; const entrada=r[3]; const salida=r[5]; const fecha=r[2]; if(!entrada || salida) continue; if((r[5]||'').toString().includes('AUTOCIERRE')) continue; try{ const fechaEntrada=new Date(fecha+'T'+entrada); if(isNaN(fechaEntrada)) continue; if((ahoraMX-fechaEntrada)/(1000*60*60)>=16){ await sClient.spreadsheets.values.update({ spreadsheetId:SPREADSHEET_ID, range:`Asistencia!F${i+2}:M${i+2}`, valueInputOption:'USER_ENTERED', requestBody:{values:[["AUTOCIERRE 16H", r[6]||'', r[7]||'', "AUTOCIERRE", "0", "8", "0", r[12]||""]]}); } }catch(e){} } }
...
async function start(){
  ...
  sock.ev.on('messages.upsert', async ({messages})=>{
    try{
      ...
      const esGrupoGerentes = GRUPOS_GERENTES.includes(jid)
      const esGrupoChecador = GRUPOS_CHECADORES.includes(jid)
      const esGrupoReportes = jid === GRUPO_REPORTES_ID
      const filtroGrupo = await getFiltroPorGrupo(jid, sock)

      // ===== NUEVA REGLA POR GRUPO =====
      if(esGrupoChecador){
        // SOLO CHECAR - bloquea todo lo demás
        if(!loc){
          const esComandoInfo = /^(numero|número|num|tel|telefono|teléfono|info|ficha|datos|dato|asistencia hoy|resumen |reporte|checador|reporte x unidad)/i.test(texto)
          if(esComandoInfo){
            await sock.sendMessage(jid,{text:`⛔ Este grupo es SOLO para checar asistencia con ubicación.\nNo se permiten consultas.`})
            return
          }
          return
        }
        // si es ubicación, deja pasar al flujo de checado abajo
      }

      if(esGrupoGerentes){
        // SOLO INFO FICHA
        if(loc){ await sock.sendMessage(jid,{text:`⛔ Este grupo es solo para consultar fichas.\nUsa: info [nombre]\nEjemplo: info fer`}); return }
        const esInfo = /^(numero|número|num|tel|telefono|teléfono|info|ficha|datos|dato)\s*(de)?/i.test(texto) || texto.trim().toLowerCase()==='id'
        if(!esInfo){
          if(/^(asistencia hoy|resumen |reporte|checador|reporte x unidad)/i.test(texto)){
            await sock.sendMessage(jid,{text:`⛔ En gerentes solo puedes usar:\n• info [nombre]\nEjemplo: info fer`})
            return
          }
          return
        }
      ...
      // ===== INFO / FICHA - SOLO GERENTES Y REPORTES =====
      if(/^(numero|número|num|tel|telefono|teléfono|info|ficha|datos|dato)\s*(de)?/i.test(texto)){
        if(esGrupoChecador){ await sock.sendMessage(jid,{text:`⛔ Este grupo es SOLO para checar con ubicación.`}); return }
      ...
      if(esGrupoGerentes || esGrupoChecador) return // ya no sigue a reporteria

      // ===== REPORTES SOLO GRUPO REPORTES =====
      ...
      // ===== CHECADO - SOLO CHECADORES Y REPORTES =====
